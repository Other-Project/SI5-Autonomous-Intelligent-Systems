...
class Pilot(Node):
    def __init__(self):
        ...
        self.goal_subscriber = self.create_subscription(
            PoseStamped, "/robot/goal_point", self.goal_callback, 10
        )
    ...
    def goal_callback(self, msg):
        self.goal_pose = msg
        self.get_logger().debug(f"Received new goal point: {msg.pose.position.x}, {msg.pose.position.y}")
    ...
    def go_to_goal(self):
        ...
        pose_global = self.tf_buffer.transform(self.goal_pose, 'map')
        ...
        if self.last_sent_goal is not None:
                ...
                if dist < self.distance_threshold:
                    self.get_logger().debug("New goal is too close to the last sent goal. Not sending a new goal.")
                    return  
